...
class _ContactSummary extends StatelessWidget { const _ContactSummary({required this.items, required this.data, required this.done}); final List<LiveDiffusionContact> items; final LiveDiffusionData data; final VoidCallback done; @override Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [const _Info(title: 'Gestion du consentement', body: 'Les contacts désinscrits ou archivés sont exclus par le backend.', icon: Icons.lock_outline_rounded), const SizedBox(height: 14), if (items.isEmpty) const _Empty(title: 'Aucun contact', body: 'Ajoutez un contact depuis le bouton + dans ce module.', icon: Icons.people_outline_rounded) else ...items.map((item) => Card(margin: const EdgeInsets.only(bottom: 8), child: ListTile(leading: CircleAvatar(child: Icon(item.optOut ? Icons.block_rounded : Icons.person_outline_rounded)), title: Text(item.name?.isNotEmpty == true ? item.name! : item.phoneE164), subtitle: Text(item.optOut ? '${item.phoneE164} · désinscrit' : item.phoneE164), trailing: IconButton(onPressed: () async { await data.updateContact(item.id, optOut: !item.optOut); done(); }, icon: Icon(item.optOut ? Icons.check_circle_outline_rounded : Icons.block_rounded)))))]); }
