  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Column(
            ...
            children: [
              ...
              SizedBox(height: 2),
              Text('Audience ciblée · validation requise',
                  style: TextStyle(
                      fontSize: 11.5,
        ...
        body: Column(
          children: [
            ...
            Expanded(
                child: FutureBuilder<List<LiveAiDiffusionRequest>>(
              future: _requestsFuture,
              builder: (_, snapshot) {
                final requests =
                    snapshot.data ?? const <LiveAiDiffusionRequest>[];
                ...
                return switch (_tab) {
                  ...
                  1 => _RequestsTab(
                      data: _data, requests: requests, onChanged: _refresh),
...
class _CreateAiDiffusionState extends State<_CreateAiDiffusion> {
  ...
  Future<void> _previewAudience() async {
    ...
    try {
      ...
      setState(() {
        ...
        if (current <= 0 || current > result.total)
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!mounted) return;
      _notice(
          'Demande soumise à validation. Aucun message n’est envoyé avant approbation.',
          success: true);
              ...
              children: [
                Center(
                    child: Container(
                        ...
                        height: 4,
                        decoration: BoxDecoration(
                            color: const Color(0xFFCDDBD6),
                            borderRadius: BorderRadius.circular(99)))),
                const SizedBox(height: 18),
                const Text('Vérifier la demande',
                    style:
                        TextStyle(fontSize: 21, fontWeight: FontWeight.w900)),
                const SizedBox(height: 10),
                _ReviewRow(
                    label: 'Audience estimée', value: '$audience contacts'),
                _ReviewRow(label: 'Quota demandé', value: '$quota messages'),
                _ReviewRow(
                    label: 'Envoi', value: 'Après validation administrateur'),
                const SizedBox(height: 12),
                const Text(
                    'La diffusion respecte les désinscriptions et aucun message ne part avant approbation.',
  ...
  void _notice(String value, {bool success = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          backgroundColor: success ? WaouhPalette.jade : null,
        ...
        children: [
          ...
          _ComposerFooter(
            ...
            onNext: _step == 0
                ? () => setState(() => _step = 1)
                : _step == 1
                    ? (_preview == null || _preview!.total == 0
                        ? _previewAudience
                        : () => setState(() => _step = 2))
                    : _submit,
          ),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            _FilterChoice(
                ...
                selected: _sources.contains('radar'),
                onTap: () => _toggle(_sources, 'radar')),
            _FilterChoice(
                label: 'Catalogue',
                icon: Icons.inventory_2_outlined,
                selected: _sources.contains('catalog'),
                onTap: () => _toggle(_sources, 'catalog')),
            _FilterChoice(
                label: 'Signaux',
                icon: Icons.notifications_active_outlined,
                selected: _sources.contains('signal'),
                onTap: () => _toggle(_sources, 'signal')),
            _FilterChoice(
                label: 'Contacts',
                icon: Icons.people_outline_rounded,
                selected: _sources.contains('wa_contact'),
                onTap: () => _toggle(_sources, 'wa_contact')),
          ]),
              ...
              children: [
                ...
                'Services',
                  ...
                  .map((value) => _FilterChoice(
                      label: value,
                      selected: _sectors.contains(value),
                      onTap: () => _toggle(_sectors, value)))
                  .toList()),
          const SizedBox(height: 20),
          ...
          Row(children: [
            Expanded(
                child: TextField(
                    ...
                    decoration:
                        const InputDecoration(hintText: 'Ex. Cotonou'))),
  ...
  Widget _buildQualificationStep() => Column(children: [
        _SurfaceCard(
          ...
          subtitle:
              'Ajustez la maturité des contacts avant de demander une validation.',
          child:
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              _FilterChoice(
                  label: 'A · Chaud',
                  selected: _classes.contains('A'),
                  onTap: () => _toggle(_classes, 'A')),
              _FilterChoice(
                  label: 'B · Tiède',
                  selected: _classes.contains('B'),
                  onTap: () => _toggle(_classes, 'B')),
              _FilterChoice(
                  label: 'C · Froid',
                  selected: _classes.contains('C'),
                  onTap: () => _toggle(_classes, 'C')),
              _FilterChoice(
                  label: 'D · Dormant',
                  selected: _classes.contains('D'),
                  onTap: () => _toggle(_classes, 'D')),
            ]),
            const SizedBox(height: 18),
            _ScoreSlider(
  ...
  Widget _buildMessageStep() => Column(children: [
        ...
        _SurfaceCard(
          ...
          subtitle:
              'Le message sera contrôlé par un administrateur avant envoi.',
          child: Column(children: [
            TextField(
                controller: _name,
                textCapitalization: TextCapitalization.sentences,
                decoration: const InputDecoration(
                    labelText: 'Nom de la diffusion',
                    hintText: 'Promotion de juillet')),
            const SizedBox(height: 12),
            TextField(
                controller: _message,
                minLines: 4,
            ...
            const Align(
                ...
                child: Text(
                    'Variables : {{display_name}}, {{ville}}, {{categorie_top}}',
                    style:
                        TextStyle(fontSize: 11.5, color: WaouhPalette.muted))),
            const SizedBox(height: 12),
            TextField(
                controller: _quota,
                keyboardType: TextInputType.number,
                decoration: InputDecoration(
                    labelText: 'Quota demandé',
                    helperText: _preview == null
                        ? null
                        : 'Maximum disponible : ${_preview!.total}')),
            const SizedBox(height: 12),
            ExpansionTile(
              tilePadding: EdgeInsets.zero,
              title: const Text('Média facultatif',
                  style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14)),
              subtitle: const Text(
                  'Ajoutez une URL image ou vidéo si nécessaire.',
              ...
              children: [
                TextField(
                    ...
                    decoration:
                        const InputDecoration(labelText: 'URL du média'))
              ],
            ),
          ]),
        ),
...
class _ComposerFooter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final icon = switch (step) {
      ...
      1 =>
        previewReady ? Icons.arrow_forward_rounded : Icons.visibility_outlined,
    ...
    return SafeArea(
      ...
      child: Container(
        ...
        decoration: const BoxDecoration(
            color: Colors.white,
            border: Border(top: BorderSide(color: Color(0xFFE0EAE6)))),
        child: Row(children: [
          if (onBack != null) ...[
            OutlinedButton.icon(
...
class _RequestsTab extends StatelessWidget {
  const _RequestsTab(
      {required this.data, required this.requests, required this.onChanged});
  final LiveDiffusionData data;
  final List<LiveAiDiffusionRequest> requests;
  final Future<void> Function() onChanged;

  Future<void> _cancel(
      BuildContext context, LiveAiDiffusionRequest request) async {
    final accepted = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Annuler la demande ?'),
        content: const Text(
            'La demande restera visible dans votre historique mais ne pourra plus être approuvée.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
        ...
        child: ListView(
          ...
          children: [
            const _InfoBanner(
              ...
              body:
                  'Chaque demande est contrôlée avant la création des envois. Les contacts désinscrits restent exclus.',
            ),
            ...
            if (requests.isEmpty)
              const _EmptyState(
                  ...
                  body:
                      'Vos demandes de diffusion apparaîtront ici après soumission.')
            else
              ...requests.map((request) => _RequestCard(
                  request: request,
                  onCancel: request.pending
...
class _RequestCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final audience = request.audienceTotal > 0
        ? request.audienceTotal
        : request.quotaRequested;
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(
                child: Text(
                    request.campaignName?.isNotEmpty == true
          ...
          Text(request.messageTemplate,
              maxLines: 2,
              overflow: TextOverflow.ellipsis,
              style: const TextStyle(color: WaouhPalette.muted, height: 1.3)),
          const SizedBox(height: 12),
          Row(children: [
            _Metric(
                icon: Icons.people_outline_rounded,
            ...
            _Metric(
                ...
                value: '${request.effectiveQuota}',
                label: 'quota'),
            const Spacer(),
            Text(_shortDate(request.createdAt),
                style:
                    const TextStyle(fontSize: 11.5, color: WaouhPalette.muted)),
          ]),
          if (request.approved || request.completed) ...[
            ...
            Text(
                '${request.quotaConsumed}/${request.effectiveQuota} messages traités',
                style:
                    const TextStyle(fontSize: 11.5, color: WaouhPalette.muted)),
          ],
          if ((request.reason ?? '').trim().isNotEmpty) ...[
            const SizedBox(height: 9),
            Text('Note : ${request.reason}',
                style: const TextStyle(
                    fontSize: 11.5,
...
class _TrackingTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final approved =
        requests.where((item) => item.approved || item.completed).length;
    final sent = requests.fold<int>(0, (sum, item) => sum + item.quotaConsumed);
    final audience =
        requests.fold<int>(0, (sum, item) => sum + item.audienceTotal);
    return ListView(
      padding: const EdgeInsets.fromLTRB(16, 16, 16, 28),
      children: [
        _MetricsGrid(values: [
        ...
        const SizedBox(height: 18),
        const Text('État des dernières diffusions',
            style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
        const SizedBox(height: 9),
        if (requests.isEmpty)
          const _EmptyState(
              icon: Icons.analytics_outlined,
              title: 'Aucune donnée à suivre',
              body: 'Les indicateurs apparaîtront après la première demande.')
        else
          ...requests.take(8).map((item) => Card(
                ...
                child: ListTile(
                  leading: CircleAvatar(
                      backgroundColor:
                          _requestColor(item.status).withOpacity(.12),
                      child: Icon(_statusIcon(item.status),
                          color: _requestColor(item.status))),
                  title: Text(
                      item.campaignName?.isNotEmpty == true
                          ? item.campaignName!
                          : 'Diffusion IA',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                  ...
                  subtitle: Text(
                      '${_requestLabel(item.status)} · ${item.quotaConsumed}/${item.effectiveQuota} traités'),
                  trailing: Text(_shortDate(item.createdAt),
                      style: const TextStyle(
                          fontSize: 11.5, color: WaouhPalette.muted)),
                ),
              )),
      ],
    );
...
class _StepIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Étape ${step + 1}/3',
          style: const TextStyle(
              fontWeight: FontWeight.w900, color: WaouhPalette.deep)),
      const SizedBox(height: 8),
      Row(
              ...
              .map((label) => Expanded(
                  child: Text(label,
                      ...
                      style: TextStyle(
                          ...
                          color: labels.indexOf(label) <= step
                              ? WaouhPalette.jade
                              : WaouhPalette.muted))))
              .toList()),
    ]);
  ...
  Widget build(BuildContext context) => FilterChip(
        ...
        avatar: selected
            ? const Icon(Icons.check_rounded,
                size: 16, color: WaouhPalette.jade)
            : (icon == null
                ? null
                : Icon(icon, size: 16, color: WaouhPalette.muted)),
        label: Text(label),
        selectedColor: const Color(0xFFE2F6EE),
        backgroundColor: Colors.white,
        side: BorderSide(
            color:
                selected ? const Color(0xFF97D9C3) : const Color(0xFFCCDAD5)),
        labelStyle: TextStyle(
            color: selected ? WaouhPalette.deep : const Color(0xFF40514B),
            fontWeight: FontWeight.w800,
            fontSize: 13),
  ...
  Widget build(BuildContext context) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text('${value.round()}',
              style: const TextStyle(
                  fontWeight: FontWeight.w900, color: WaouhPalette.jade))
        ]),
        Slider(
            value: value,
            min: 0,
...
class _AudiencePreviewCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: const Color(0xFFC5EAD8)),
            borderRadius: BorderRadius.circular(18)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(
                ...
                height: 42,
                decoration: const BoxDecoration(
                    color: Color(0xFF08756A), shape: BoxShape.circle),
                child:
            ...
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  Text('${preview.total} contacts estimés',
                      style: const TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.w900,
                          color: WaouhPalette.deep)),
                  const Text('Aucun numéro complet n’est exposé.',
                      style:
                          TextStyle(fontSize: 11.5, color: WaouhPalette.muted)),
                ])),
          ]),
          if (preview.breakdown.isNotEmpty) ...[
            ...
            Wrap(
                spacing: 8,
                runSpacing: 8,
                children: preview.breakdown.entries
                    .map((entry) =>
                        _BreakdownPill(label: '${entry.key} · ${entry.value}'))
                    .toList()),
          ],
          if (preview.sample.isNotEmpty) ...[
            const SizedBox(height: 12),
            Text(
                'Exemple masqué : ${preview.sample.first['phone_masked'] ?? '—'}',
                style: const TextStyle(
                    fontSize: 11.5,
                    color: WaouhPalette.muted,
                    fontWeight: FontWeight.w700)),
...
class _BreakdownPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(99),
          border: Border.all(color: const Color(0xFFC9E8DB))),
      child: Text(label,
          style: const TextStyle(
              fontSize: 11.5,
              fontWeight: FontWeight.w800,
...
class _ReviewRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
      ...
      child: Row(children: [
        Text(label, style: const TextStyle(color: WaouhPalette.muted)),
        const Spacer(),
...
class _InfoBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(color: const Color(0xFFF1DCAB)),
          borderRadius: BorderRadius.circular(16)),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Icon(icon, color: WaouhPalette.orange),
        const SizedBox(width: 10),
        Expanded(
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(body,
              style: const TextStyle(
                  fontSize: 12, color: WaouhPalette.muted, height: 1.3))
        ]))
      ]));
}
...
class _Metric extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(icon, size: 15, color: WaouhPalette.jade),
        const SizedBox(width: 4),
        Text('$value ',
            style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 12)),
        Text(label,
            style: const TextStyle(fontSize: 11.5, color: WaouhPalette.muted))
      ]);
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
      ...
      child: Column(children: [
        Icon(icon, size: 52, color: WaouhPalette.muted),
      ...
      child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.cloud_off_rounded,
                size: 52, color: WaouhPalette.muted),
            const SizedBox(height: 12),
            const Text('Diffusion indisponible',
