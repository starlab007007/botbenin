            ...
            Flexible(
              child: SingleChildScrollView(
                padding: EdgeInsets.fromLTRB(
                    20, 18, 20, 24 + MediaQuery.viewInsetsOf(context).bottom),
                child: child,
              ),
            ),
          ]),
...
class _CreateSheetState extends State<_CreateSheet> {
  late final TextEditingController _name = TextEditingController(
    text:
        'session-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}',
  );
  @override
  void dispose() {
    _name.dispose();
  ...
  Widget build(BuildContext context) => WhatsAppSheetFrame(
        ...
        child: Column(children: [
          _ActionRow(
              ...
              onTap: () =>
                  Navigator.pop(context, LiveWhatsAppSessionAction.connect)),
          if (!session.isWorking)
            _ActionRow(
                icon: Icons.play_arrow_rounded,
                title: 'Démarrer la session',
                onTap: () =>
                    Navigator.pop(context, LiveWhatsAppSessionAction.start)),
          if (session.isWorking)
            _ActionRow(
                icon: Icons.stop_circle_outlined,
                title: 'Arrêter la session',
                onTap: () =>
                    Navigator.pop(context, LiveWhatsAppSessionAction.stop)),
          if (session.isWorking)
            _ActionRow(
                icon: Icons.send_rounded,
                title: 'Envoyer un message test',
                onTap: () =>
                    Navigator.pop(context, LiveWhatsAppSessionAction.test)),
          _ActionRow(
              icon: Icons.smart_toy_outlined,
              title: 'Lier à un bot',
              onTap: () =>
                  Navigator.pop(context, LiveWhatsAppSessionAction.linkBot)),
          _ActionRow(
              icon: Icons.webhook_outlined,
              title: 'Configurer un webhook',
              onTap: () =>
                  Navigator.pop(context, LiveWhatsAppSessionAction.webhook)),
          _ActionRow(
              icon: Icons.delete_outline_rounded,
              title: 'Supprimer la session',
              danger: true,
              onTap: () =>
                  Navigator.pop(context, LiveWhatsAppSessionAction.delete)),
        ]),
      );
}
