...
class _LiveInboxScreenV2State extends State<LiveInboxScreenV2> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<LiveStatus>>(
      ...
      builder: (_, statusSnapshot) {
        final statuses = (statusSnapshot.data ?? const <LiveStatus>[])
            .where((item) => item.active)
            .toList();
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: _tab == 2
                      ...
                      : 'Rechercher discussions, statuts...',
            ...
            Padding(
              ...
              child: SegmentedButton<int>(
                segments: [
                  ...
                  ButtonSegment(
                      value: 1, label: _StatusTabLabel(count: newStatusCount)),
                  const ButtonSegment(
                      value: 2,
                ...
                onSelectionChanged: (value) =>
                    _selectTab(value.first, statuses),
              ),
            ),
            Expanded(
              child: switch (_tab) {
  ...
  PreferredSizeWidget _inboxAppBar(BuildContext context,
      legacy.AuthController auth, LiveWaouhController controller) {
      ...
      title: Column(
          ...
          children: [
            ...
            Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(auth.signedIn ? 'WAOUH actif' : 'Mode invité',
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<LiveMatch>>(
      ...
      builder: (_, activeMatchSnapshot) => StreamBuilder<List<LiveMatch>>(
        ...
        builder: (_, archivedMatchSnapshot) =>
            StreamBuilder<List<LiveConversation>>(
          stream: controller.conversations(archived: false),
          builder: (_, activeConversationSnapshot) =>
              StreamBuilder<List<LiveConversation>>(
            stream: controller.conversations(archived: true),
            builder: (_, archivedConversationSnapshot) {
              final activeMatches = (activeMatchSnapshot.data ??
                      const <LiveMatch>[])
                  .where((item) => matches(
                      '${item.title} ${item.city ?? ''} ${item.seedText ?? ''} ${item.price ?? ''}'))
                  .toList();
              final archivedMatches = (archivedMatchSnapshot.data ??
                      const <LiveMatch>[])
                  .where((item) => matches(
                      '${item.title} ${item.city ?? ''} ${item.seedText ?? ''} ${item.price ?? ''}'))
                  .toList();
              final activeConversations = (activeConversationSnapshot.data ??
                      const <LiveConversation>[])
                  .where((item) => matches(
                      '${item.phoneNumber ?? ''} ${item.lastMessage ?? ''}'))
                  .toList();
              final archivedConversations = (archivedConversationSnapshot
                          .data ??
                      const <LiveConversation>[])
                  .where((item) => matches(
                      '${item.phoneNumber ?? ''} ${item.lastMessage ?? ''}'))
              ...
              final currentConversations =
                  archives ? archivedConversations : activeConversations;
              final activeTotal =
                  activeMatches.length + activeConversations.length;
              final archivedTotal =
                  archivedMatches.length + archivedConversations.length;
              final unreadTotal = activeMatches.fold<int>(
                  0, (total, item) => total + item.unreadCount);
              final empty =
                  currentMatches.isEmpty && currentConversations.isEmpty;
                  ...
                  Row(children: [
                    Expanded(
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                          Text(archives ? 'ARCHIVES' : 'CONVERSATIONS PRODUIT',
                              style: const TextStyle(
                    ...
                    OutlinedButton.icon(
                        ...
                        label: Text(archives
                            ? 'Actives ($activeTotal)'
                            : 'Archives ($archivedTotal)')),
                  ]),
                  const SizedBox(height: 8),
                  if (currentMatches.isNotEmpty)
                    ...currentMatches.map(
                        (match) => _MatchRow(match: match, archived: archives)),
                  if (currentConversations.isNotEmpty)
                    ...currentConversations.map((conversation) =>
                        _ConversationRow(
                            conversation: conversation, archived: archives)),
                  if (empty)
                    _EmptyConversations(
                        archived: archives, onNewChat: onNewChat),
                ],
              );
            },
          ),
        ...
        child: Padding(
          ...
          child:
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              ...
              _IntentChip(
                  label: 'Acheter', icon: Icons.search_rounded, onTap: onBuy),
              _IntentChip(
                  label: 'Négocier',
    ...
    return Card(
      ...
      child: ListTile(
        ...
        onTap: () async {
          ...
          if (context.mounted) await livePushMatchChat(context, match);
        },
        ...
        title: Row(children: [
          Expanded(
              child: Text(match.title,
                  ...
                  style: TextStyle(
                      fontWeight: FontWeight.w900,
                      color: unread > 0 ? legacy.WaouhColors.ink : null))),
          if (unread > 0)
            const Icon(Icons.circle, color: Color(0xFF24E58F), size: 10)
        ]),
        subtitle: Text(
            '${match.role == 'seller' ? 'Acheteur intéressé' : 'Nouvelle annonce correspondante'}${match.city == null ? '' : ' · ${match.city}'}\n$when',
            maxLines: 2,
            overflow: TextOverflow.ellipsis,
            style: TextStyle(
          ...
          child: ListTile(
            ...
            trailing: archived
                ? const Icon(Icons.archive_outlined,
                    color: legacy.WaouhColors.muted)
                : Text(_relativeMatchTime(conversation.updatedAt),
                    style: const TextStyle(
      ...
      child: Padding(
          ...
          child: Column(children: [
            const Icon(Icons.forum_outlined,
                size: 48, color: legacy.WaouhColors.muted),
            ...
            Text(
                archived
                    ...
                    : 'Utilisez WAOUH pour commencer une recherche, une vente ou une négociation.',
...
class _ProfileAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fallback = CircleAvatar(
        ...
        child: Text(initials.isEmpty ? 'W' : initials,
            style: const TextStyle(
                color: legacy.WaouhColors.jade, fontWeight: FontWeight.w900)));
    return Center(
        child: InkResponse(
            onTap: onTap,
            radius: 28,
...
class _IconBadgeButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Stack(clipBehavior: Clip.none, children: [
        ...
        if (count > 0)
          Positioned(
              top: 4, right: 4, child: _CountBubble(count: count, small: true))
      ]);
}
...
class _OnlineDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      height: 8,
      decoration: const BoxDecoration(
          color: Color(0xFF24E58F), shape: BoxShape.circle));
}
