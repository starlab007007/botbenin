    ...
    try {
      ...
      if (mounted) {
        setState(() => _snapshot = LiveRadarScanSnapshot(
              ...
              catalogCount: 0,
              statusCount: 0,
              warning: 'Le Radar ne répond pas. Vérifiez la connexion puis relancez.',
            ));
      }
    } finally {
      _sweep.stop();
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          Row(children: [
            const Expanded(child: Text('Opportunités détectées', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900))),
            Text('${items.length}', style: const TextStyle(color: Color(0xFF08756A), fontSize: 17, fontWeight: FontWeight.w900)),
          ]),
          const SizedBox(height: 10),
          if (_loading && snapshot == null)
            const Padding(padding: EdgeInsets.symmetric(vertical: 42), child: Center(child: CircularProgressIndicator()))
          else if (items.isEmpty)
            RadarEmpty(radiusKm: _filters.maxRadiusKm, onFilters: _openFilters, onScan: () => _scan(refreshLocation: true))
          else
