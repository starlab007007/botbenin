...
test -f scripts/repair_compile_errors.py || fail "Script de réparation absent."
grep -q '^name: waouh_app_native$' pubspec.yaml || fail "Mauvais projet Flutter."

# The test build deliberately uses the automatic Android debug certificate;
# WAOUH_TEST_RELEASE=1 is handled by android/app/build.gradle.
