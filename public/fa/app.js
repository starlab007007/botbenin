(() => {
  ...
  const K = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im12eW5lcHF1bGhmbHh0eXltdHpzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDc1OTgxNTMsImV4cCI6MjA2MzEzNDE1M30.g1llr-Q6T3h06xFV7hCNRWZHG20wQHoBmp5zL0OAKh8';
  ...
  const F = [
    ['comprehensive', 'Comprendre le signe en profondeur', 'Expliquer le sens central, les ouvertures, les vigilances, les conditions d’évolution et la conduite juste.'],
    ['positive', 'Lumière et ouvertures', 'Développer uniquement les forces, protections, ouvertures et conditions favorables.'],
    ['warning', 'Vigilances et obstacles', 'Identifier les risques, blocages et attitudes aggravantes sans fatalité.'],
    ['relationship', 'Amour, famille et relations', 'Interpréter les liens, la famille, le couple, la parole et la confiance.'],
    ['work', 'Travail, argent et projets', 'Interpréter le travail, les ressources, le commerce, les projets et les conditions concrètes.'],
    ['health', 'Santé et équilibre', 'Présenter seulement la portée symbolique sur l’équilibre sans diagnostic.'],
    ['action', 'Conseils et conduite à tenir', 'Transformer le message en attitudes concrètes sans détailler de rituel réservé.'],
    ['summary', 'Résumé essentiel', 'Donner une synthèse courte : force, vigilance, condition et orientation.'],
  ...
  function write(entries) { localStorage.setItem(J, JSON.stringify({ version: '5.2', entries: entries.slice(0, 100) })); }
  function entry(id) { return list().find((x) => x.id === id); }
  function save() { if (!s.id) return; const entries = list(); const item = { id: s.id, date: s.date, cat: s.cat, raw: s.raw, intent: s.intent, faces: s.faces, sign: { ref: s.sign.ref, name: s.sign.name, x: s.sign.x.n, y: s.sign.y.n }, msg: s.msg, fav: entry(s.id)?.fav || false }; const index = entries.findIndex((x) => x.id === s.id); if (index >= 0) entries[index] = item; else entries.unshift(item); write(entries); }
