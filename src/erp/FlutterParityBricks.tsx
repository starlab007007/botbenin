...
const QUICK_AGENTS: QuickAgent[] = [
  ...
  },
  {
    id: 'stock',
    label: 'Gestion stock',
    desc: 'Produits, alertes et réapprovisionnement intelligent',
    icon: Package,
...
export function WhatsAppFlutterParityBrick() {
  ...
  const { sessions, loading, createSession, startSession, stopSession, refreshData } = useWAHADashboard();
  ...
  const merged: WAHASession[] = useMemo(() => {
    const allowed = new Set(dbSessions.map((row) => row.session_name));
    const map = new Map<string, WAHASession>();
    for (const row of dbSessions) {
      map.set(row.session_name, {
        name: row.session_name,
        status: (row.status as any) || 'STOPPED',
        server: 'WAHA',
        config: { metadata: { phone_number: row.phone_number ?? undefined } },
      });
    }
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><QrCode className="h-5 w-5 text-amber-500" /> QR WhatsApp</CardTitle></CardHeader>
          <CardContent className="text-sm text-muted-foreground">Créez ou démarrez une session, puis scannez le QR depuis WhatsApp → Appareils connectés.</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Bot className="h-5 w-5 text-emerald-600" /> Agent IA lié</CardTitle></CardHeader>
          <CardContent className="text-sm text-muted-foreground">Chaque session peut être reliée à un agent pour répondre automatiquement selon votre catalogue.</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Send className="h-5 w-5 text-blue-600" /> Message test</CardTitle></CardHeader>
          <CardContent className="text-sm text-muted-foreground">Une fois connecté, testez l’envoi et vérifiez le webhook avant exploitation.</CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {loading && merged.length === 0 ? (
            <div className="flex items-center justify-center py-10 text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Chargement…</div>
          ) : merged.length === 0 ? (
            <div className="rounded-2xl border border-dashed p-8 text-center">
              ...
              <Button className="mt-4 bg-[#25D366] hover:bg-[#1da851]" onClick={quickCreate}><Plus className="mr-2 h-4 w-4" /> Créer ma première session</Button>
            </div>
          ) : (
            merged.map((session) => {
              ...
              return (
                <div key={session.name} className="flex flex-col gap-3 rounded-2xl border bg-white p-4 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {!isWorking ? (
                      <Button variant="outline" size="sm" onClick={() => startSession(session.name)}><Play className="mr-1 h-3.5 w-3.5" /> Démarrer</Button>
                    ) : (
...
export function DiffusionFlutterParityBrick() {
  ...
  const tabs: Array<{ key: DiffusionTab; label: string; icon: LucideIcon }> = [
    ...
    >
      <div className="flex flex-wrap gap-2 rounded-3xl border bg-[hsl(165_91%_18%)] p-2">
        {tabs.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => setTab(key)} className={cn('flex items-center gap-2 rounded-2xl px-4 py-2 text-sm font-semibold transition', tab === key ? 'bg-white text-[hsl(165_91%_18%)]' : 'text-white/85 hover:bg-white/10')}>
            <Icon className="h-4 w-4" /> {label}
          </button>
        ))}
      </div>

      {tab === 'ia' && <Card><CardContent className="p-4"><AudienceBuilder onSubmitted={() => setTab('campaigns')} /></CardContent></Card>}
      {tab === 'campaigns' && (
        <Card>
          ...
          <CardContent className="space-y-3">
            {s.all.length === 0 && <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900"><strong>Aucune session WhatsApp connectée.</strong> Configurez une session avant l’envoi.</div>}
            {d.campaigns.length === 0 ? (
              ...
              return (
                <div key={campaign.id} className="rounded-2xl border bg-white p-4">
                  <div className="flex items-start justify-between gap-2"><div><strong>{campaign.name}</strong><p className="text-xs text-muted-foreground">{campaign.type} · {new Date(campaign.created_at).toLocaleString('fr-FR')}</p></div><Badge>{campaign.status}</Badge></div>
      ...
      {tab === 'contacts' && <Card><CardHeader><CardTitle>Contacts</CardTitle></CardHeader><CardContent><div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">Contacts WhatsApp et audiences de diffusion gérés par le même backend que Flutter.</div></CardContent></Card>}
      {tab === 'sessions' && (
        <Card>
          ...
          <CardContent className="space-y-3">
            {s.loading ? <div className="py-8 text-center text-sm text-muted-foreground"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" /> Chargement…</div> : s.all.length === 0 ? <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">Aucune session de diffusion configurée.</div> : s.all.map((row) => <div key={row.id} className="rounded-2xl border bg-white p-4"><strong>{row.session_name}</strong><p className="text-xs text-muted-foreground">{row.phone_number || 'Aucun numéro'} · {row.status || 'session'}</p></div>)}
          </CardContent>
        </Card>
      )}
...
export function BotsFlutterParityBrick() {
  ...
  return (
    <BrickFrame
      ...
      icon={Bot}
      action={<Button className="bg-[hsl(165_91%_25%)] hover:bg-[hsl(165_91%_18%)]" onClick={() => navigate('/app/bots/new')}><Plus className="mr-2 h-4 w-4" /> Nouvel agent</Button>}
    >
      <Card>
        <CardHeader><CardTitle>Créer un agent</CardTitle></CardHeader>
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {QUICK_AGENTS.map((agent) => {
              const Icon = agent.icon;
              return (
                <button key={agent.id} onClick={() => navigate(agent.route)} className={cn('rounded-2xl bg-gradient-to-br p-4 text-left text-white shadow-sm transition active:scale-[0.98]', agent.color)}>
