...
export default function WaouhMusePage({ embedded = false }: WaouhMusePageProps) {
  ...
  const buyWithMuse = () => navigate("/app/avatar/journey/buy");
  const sellWithMuse = () => navigate("/app/avatar/journey/sell");
  const askAvatar = () => navigate("/app/avatar/journey/ask");
  ...
  return (
    ...
    >
      ...
      <section className={cn("mx-auto w-full space-y-3", embedded ? "max-w-none p-3 lg:p-4" : "max-w-7xl px-3 py-4 sm:px-4")}>
        {section !== "chat" ? (
          <div className="relative overflow-hidden rounded-[28px] border border-blue-100/80 bg-gradient-to-br from-[#eef5ff] via-[#f7f9ff] to-[#f5f1ff] p-5 text-slate-950 shadow-[0_24px_70px_-38px_rgba(79,127,255,.38)] sm:p-6">
            <div className="pointer-events-none absolute -right-24 -top-28 h-72 w-72 rounded-full bg-cyan-300/25 blur-3xl" />
            ...
            <div className="relative grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)] xl:items-end">
              <div className="min-w-0">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <Badge className="border-blue-100 bg-white/80 text-blue-700 hover:bg-white/80">
                    <BrainCircuit className="mr-1 h-3.5 w-3.5" /> WAOUH Avatar
                  </Badge>
                  <Badge className="border-cyan-100 bg-cyan-50 text-cyan-700 hover:bg-cyan-50">
                    <Sparkles className="mr-1 h-3.5 w-3.5" /> NEXUS actif
                  </Badge>
                  <Badge className="border-violet-100 bg-violet-50 text-violet-700 hover:bg-violet-50">
                    Signal Fabric
                  </Badge>
                </div>

                <h1 className="max-w-3xl text-2xl font-black leading-[1.08] tracking-[-0.035em] sm:text-3xl lg:text-[34px]">
                  Un objectif. Votre Avatar conduit. NEXUS trouve le marché.
                </h1>
                ...
                <div className="mt-5 grid max-w-2xl gap-2 sm:grid-cols-2">
                  <button
                    type="button"
                    onClick={buyWithMuse}
                    className="group flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left text-slate-950 shadow-lg shadow-black/10 transition hover:-translate-y-0.5 hover:shadow-xl"
                  >
                  ...
                  <button
                    ...
                    className="group flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-left text-slate-950 backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/15"
                  >
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-amber-50 text-amber-600"><ShoppingBag className="h-5 w-5" /></span>
                    <span className="min-w-0 flex-1">
                      <span className="block text-xs font-black">Vendre intelligemment</span>
                      <span className="mt-0.5 block text-[10px] font-semibold text-slate-500">Trouver et qualifier les acheteurs</span>
                    </span>
                    <ArrowRight className="h-4 w-4 text-slate-400 transition group-hover:translate-x-0.5" />
                  </button>
                  <button
                    type="button"
                    onClick={askAvatar}
                    className="group flex items-center gap-3 rounded-2xl border border-violet-100 bg-violet-50/80 px-4 py-3 text-left text-slate-950 transition hover:-translate-y-0.5 hover:bg-violet-100/70 sm:col-span-2"
                  >
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-violet-600 shadow-sm"><MessageSquareText className="h-5 w-5" /></span>
                    <span className="min-w-0 flex-1">
                      <span className="block text-xs font-black">Demander à mon Avatar</span>
                      <span className="mt-0.5 block text-[10px] font-semibold text-slate-500">Comprendre l’objectif et choisir le meilleur parcours</span>
                    </span>
                    <ArrowRight className="h-4 w-4 text-slate-400 transition group-hover:translate-x-0.5" />
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 rounded-[22px] border border-blue-100 bg-white/55 p-2.5 backdrop-blur-sm">
                {[
                  [MessageSquareText, "Objectif", "Parlez naturellement"],
                  [BrainCircuit, "Muse", "Conduit et poursuit"],
                  [Target, "NEXUS", "Trouve offre ↔ demande"],
                  [Network, "Signal", "Classe et sécurise"],
                ].map(([Icon, title, subtitle]) => (
                  <div key={String(title)} className="rounded-2xl border border-slate-200 bg-white/80 p-3">
                    <Icon className="h-4 w-4 text-blue-600" />
                    <div className="mt-2 text-[11px] font-black">{String(title)}</div>
                    <div className="mt-0.5 text-[9px] font-semibold text-slate-400">{String(subtitle)}</div>
                  </div>
                ))}
              </div>
            </div>
