...
import { WaouhAvatarCommerceFlow } from "@/components/waouh/WaouhAvatarCommerceFlow";

type WaouhAvatarPageProps = {
  embedded?: boolean;
};
...
export default function WaouhAvatarPage({ embedded = false }: WaouhAvatarPageProps) {
  ...
  const buyWithAvatar = () => setCommerceIntent("buy");
  const sellWithAvatar = () => setCommerceIntent("sell");
  const askAvatar = () => setCommerceIntent("ask");

  if (commerceIntent) {
    return (
      <main className={cn("waouh-avatar-page bg-white", embedded ? "h-full min-h-0" : "min-h-screen")}>
        <WaouhAvatarCommerceFlow
          intent={commerceIntent}
          avatarName={avatarName}
          onExit={() => setCommerceIntent(null)}
        />
      </main>
    );
  }
              ...
              <div className="min-w-0">
                ...
                <div className="mt-5 grid max-w-3xl gap-2 sm:grid-cols-3">
                  ...
                  <button
                    ...
                    className="group flex items-center gap-3 rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-left text-white backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/15"
                  >
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white/12 text-violet-50"><MessageSquareText className="h-5 w-5" /></span>
                    <span className="min-w-0 flex-1">
                      <span className="block text-xs font-black">Demander à l’Avatar</span>
                      <span className="mt-0.5 block text-[10px] font-semibold text-white/70">Explorer, comparer ou organiser</span>
                    </span>
                    <ArrowRight className="h-4 w-4 text-white/55 transition group-hover:translate-x-0.5" />
                  </button>
                </div>
