...
export const useLocalBusinesses = () => {
  ...
  const fetchBusinesses = async (sessionId?: string) => {
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger les entreprises locales",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const saveBusinesses = async (businessesData: Omit<LocalBusiness, 'id' | 'created_at'>[], sessionId: string) => {
    try {
      ...
      console.error('Error saving local businesses:', error);
      toast({
        title: "Erreur",
