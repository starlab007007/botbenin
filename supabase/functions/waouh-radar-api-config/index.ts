...
const CONTACT_RANK: Record<ContactabilityLevel, number> = { C0: 0, C1: 1, C2: 2, C3: 3, C4: 4 };

function maxContactability(row: any): ContactabilityLevel {
  if (!row?.supports_contact) return "C0";
  const caps = safeExtra(row?.capabilities);
  if (row?.family === "partner" || caps.agent_to_agent === true) return "C4";
  if (caps.opt_in_required === true || ["sms_rcs", "ussd"].includes(String(row?.source_key || ""))) return "C3";
  if (["internal", "messaging", "telephony"].includes(String(row?.family || "")) || row?.source_key === "qr") return "C2";
  return "C1";
}

function allowedContactability(row: any): ContactabilityLevel[] {
  const max = maxContactability(row);
  return CONTACT_LEVELS.filter((level) => CONTACT_RANK[level] <= CONTACT_RANK[max]);
}

async function contactPolicyRows(admin: any) {
  const { data, error } = await admin.from("waouh_discovery_sources")
    .select("source_key,label,family,connector_mode,operational_state,supports_contact,default_contactability,capabilities,metadata,updated_at")
    .order("family").order("label");
  ...
  return (data || []).map((row: any) => ({
    ...
    supports_contact: row.supports_contact === true,
    default_contactability: row.default_contactability || "C0",
    max_contactability: maxContactability(row),
    allowed_levels: allowedContactability(row),
    capabilities: safeExtra(row.capabilities),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "contact_policy_list") {
      return json({
        ok: true,
        levels: CONTACT_LEVELS,
        policies: await contactPolicyRows(admin),
        rules: {
          C0: "Découverte uniquement : aucun contact révélé et aucun outreach automatique.",
          C1: "Contact professionnel public : révélation possible, automatisation interdite par défaut.",
          C2: "Conversation privée / blind matching : contact masqué, approbation requise.",
          C3: "Opt-in commercial : contact utilisable selon consentement et règles d’approbation.",
          C4: "Agent ↔ Agent / partenaire WAOUH : orchestration autorisée dans le cadre contractuel.",
        },
      });
    }

    if (action === "contact_policy_update") {
      const sourceKey = String(body.source_key || "").trim();
      const requested = String(body.default_contactability || "").trim() as ContactabilityLevel;
      if (!sourceKey) return jsonError("source_key requis", 400);
      if (!CONTACT_LEVELS.includes(requested)) return jsonError("Niveau C0–C4 invalide", 400);

      const { data: before, error: beforeError } = await admin.from("waouh_discovery_sources")
        .select("source_key,label,family,connector_mode,operational_state,supports_contact,default_contactability,capabilities,metadata,updated_at")
        .eq("source_key", sourceKey).maybeSingle();
      if (beforeError) throw beforeError;
      if (!before) return jsonError("Source NEXUS inconnue", 404);

      const allowed = allowedContactability(before);
      if (!allowed.includes(requested)) {
        return jsonError(`Niveau ${requested} interdit pour ${before.label}. Maximum sûr : ${maxContactability(before)}.`, 400);
      }

      const existingMeta = safeExtra(before.metadata);
      const nextMetadata = {
        ...existingMeta,
        contact_policy: {
          default_contactability: requested,
          max_contactability: maxContactability(before),
          updated_at: new Date().toISOString(),
          updated_by: userId,
        },
      };
      const { data: after, error: updateError } = await admin.from("waouh_discovery_sources")
        .update({
          default_contactability: requested,
          metadata: nextMetadata,
          updated_at: new Date().toISOString(),
        })
        .eq("source_key", sourceKey).select("*").single();
      if (updateError) throw updateError;

      const { error: auditError } = await admin.from("waouh_admin_control_audit").insert({
        module_key: "nexus",
        actor_id: userId,
        action: "contactability_update",
        before_state: { source_key: sourceKey, default_contactability: before.default_contactability },
        after_state: { source_key: sourceKey, default_contactability: requested, max_contactability: maxContactability(before) },
      });
      if (auditError) console.warn("[waouh-radar-api-config] contact policy audit", auditError);
      ...
      return json({
        ...
        policy: {
          ...
          label: after.label,
          family: after.family,
          operational_state: after.operational_state,
          supports_contact: after.supports_contact,
          default_contactability: after.default_contactability,
          max_contactability: maxContactability(after),
          allowed_levels: allowedContactability(after),
          capabilities: safeExtra(after.capabilities),
    ...
    if (action === "upsert") {
      ...
      const { data: existing } = await admin.from("waouh_radar_api_configs")
        .select("source_key,label,auth_mode").eq("provider", provider).maybeSingle();
      const apiKey = typeof body.api_key === "string" ? body.api_key.trim() : undefined;
      const extraConfig = safeExtra(body.extra_config);
      const patch: any = {
        provider,
        source_key: body.source_key || existing?.source_key || provider,
        label: body.label || existing?.label || PROVIDERS[provider].label,
        auth_mode: existing?.auth_mode || (PROVIDERS[provider].keyRequired ? "api_key" : "native"),
        updated_by: userId,
      };
