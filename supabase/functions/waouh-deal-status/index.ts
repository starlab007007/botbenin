...
Deno.serve(async (req) => {
  ...
  try {
    const { deal_id, status } = await req.json();
    ...
    if (status === "picked_up") {
      ...
      const text = `⚠️ *Livraison annulée* pour « ${title} ». L'équipe WAOUH vous recontactera.`;
      await Promise.all([
        insertInAppNotif(sb, deal.buyer_user_id, deal.article_id, "deal_cancelled", text, { deal_id }),
        insertInAppNotif(sb, deal.seller_user_id, deal.article_id, "deal_cancelled", text, { deal_id }),
      ]);
    }
