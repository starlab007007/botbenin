-- CORRECTION DÉFINITIVE DES PROBLÈMES DE SAUVEGARDE DE BOTS
...
-- 4. Assurer que tous les utilisateurs existants ont un bot_owner
INSERT INTO public.bot_owners (user_id, max_bots, subscription_plan)
SELECT u.id, 10, 'free'
FROM auth.users u
...
-- 5. Optimiser les politiques RLS pour éviter les conflits
...
-- 6. Assurer que la contrainte UNIQUE existe pour éviter les doublons
ALTER TABLE public.bot_owners 
ADD CONSTRAINT IF NOT EXISTS bot_owners_user_id_unique UNIQUE (user_id);
