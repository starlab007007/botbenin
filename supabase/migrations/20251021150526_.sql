-- Correction complète pour enregistrer TOUS les champs des prospects
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Insérer le prospect avec TOUS les champs
      INSERT INTO prospects (
        ...
        v_business.email,
        v_business.phone,
        v_business.job_title, -- Mapping du job_title vers position
        'new',
        'local_search',
        CASE 
        ...
        END,
        CASE 
          WHEN v_business.address IS NOT NULL AND v_business.address != 'Localisation non précisée'
          THEN 'Adresse: ' || v_business.address
          ELSE NULL
        END,
        -- TOUS les champs dans custom_fields
        jsonb_build_object(
          ...
          'company_size', v_business.company_size,
          'linkedin_url', v_business.linkedin_url,
