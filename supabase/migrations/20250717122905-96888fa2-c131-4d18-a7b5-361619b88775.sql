...
-- 6. Test de la fonction pour s'assurer qu'elle fonctionne
DO $$
DECLARE
  test_user_id uuid;
  test_owner_id uuid;
BEGIN
  ...
  -- Tester la fonction get_or_create_bot_owner
  test_owner_id := public.get_or_create_bot_owner(test_user_id);
  
  -- Vérifier que ça marche bien
  IF test_owner_id IS NULL THEN
    RAISE EXCEPTION 'La fonction get_or_create_bot_owner a échoué';
  END IF;
  
  -- Nettoyer le test
  DELETE FROM public.bot_owners WHERE user_id = test_user_id;
  
  RAISE NOTICE 'Test réussi: la fonction get_or_create_bot_owner fonctionne correctement';
END;
$$;